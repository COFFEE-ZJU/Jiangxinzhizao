...
AccountShowController = RouteController.extend({
  ...
  sort: {submitted: -1, _id: -1},
  subscriptions: function() {
    this.postsSub = Meteor.subscribe('relatedPosts', Meteor.userId());
  },
  publishedPosts: function() {
    return Posts.find({userId: Meteor.userId()}, {sort: this.sort});
  },
  recommendedPosts: function() {
    return Posts.find({upvoters: Meteor.userId()}, {sort: this.sort});
  },
  data: function() {
    var hasMore = false;
    return {
...
Router.route('/account', {name: 'accountShow'});
